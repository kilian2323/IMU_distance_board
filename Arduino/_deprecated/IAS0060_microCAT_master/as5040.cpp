...
AS5040::AS5040 (byte pinCLK, byte pinDO, byte pinPROG) 
{
  _pinCLK  = pinCLK ;
//  _pinCS   = pinCS ;
  _pinDO   = pinDO ;
...
void AS5040::initialize (byte pinCS)
{
  _pinCS  = pinCS ;
}
...
boolean AS5040::begin (byte mode, boolean reverse, unsigned int offset)
{
  ...
  byte count = 0 ;
  while (singleRead (), (_status & AS5040_STATUS_OCF) == 0)
  {
    if (count > 30)
...
signed int AS5040::singleRead () {
  SPI.setDataMode(SPI_MODE3);
  digitalWrite (_pinCS, LOW) ;
  signed int value = 0 ;
  unsigned int receivedVal16 = SPI.transfer16(0x0000);
  value = (receivedVal16 >> 5);
  value = value-512;
  byte status = 0 ;
  status = receivedVal16 & 0x001F;
  _parity = even_parity (value >> 2) ^ even_parity (value & 3) ^ even_parity (status) ;
  _status = status ; // >> 1
  digitalWrite (_pinCS, HIGH) ;
